...
let push_rel_context_eqn sign eqn =
  {eqn with
     rhs = {eqn.rhs with rhs_env = push_rel_context sign eqn.rhs.rhs_env} }

let push_rel_context_eqn_with_names sign eqn =
...
let push_alias_eqn alias eqn =
  ...
  push_rel_context_eqn [alias] eqn
...
let build_branch current realargs deps (realnames,curname) pb arsign eqns const_info =
  ...
  { pb with
      ...
      pred = pred;
      history = history;
      mat = List.map (push_rel_context_eqn_with_names typs) submat }
